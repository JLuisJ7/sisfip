...
#
# Structure for table "informeensayo"
#

CREATE TABLE `informeensayo` (
  `nroInforme` int(10) unsigned NOT NULL,
  `codInforme` varchar(100) DEFAULT NULL,
  `nroReporte` int(10) unsigned DEFAULT NULL,
  `nroOrden` int(10) unsigned DEFAULT NULL,
  `nroSolicitud` int(10) unsigned DEFAULT NULL,
  ...
  `eliminado` char(1) DEFAULT '0',
  `fecha_registro` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY `fk_informe_reporteensayo` (`nroReporte`),
  KEY `fk_informe_ordentrabajo` (`nroOrden`),
  KEY `fk_informe_solicitud` (`nroSolicitud`),
  KEY `fk_informe_Cliente` (`idCliente`),
  KEY `fk_informe_Muestra` (`idMuestra`),
  CONSTRAINT `fk_informe_Cliente` FOREIGN KEY (`idCliente`) REFERENCES `cliente` (`idCliente`),
  CONSTRAINT `fk_informe_Muestra` FOREIGN KEY (`idMuestra`) REFERENCES `muestra` (`idMuestra`),
  CONSTRAINT `fk_informe_ordentrabajo` FOREIGN KEY (`nroOrden`) REFERENCES `ordentrabajo` (`nroOrden`),
  CONSTRAINT `fk_informe_reporteensayo` FOREIGN KEY (`nroReporte`) REFERENCES `reporteensayo` (`nroEnsayo`),
  CONSTRAINT `fk_informe_solicitud` FOREIGN KEY (`nroSolicitud`) REFERENCES `solicitud` (`nroSolicitud`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

#
# Data for table "informeensayo"
#
